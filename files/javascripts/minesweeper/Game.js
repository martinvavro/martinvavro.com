...
class GameState {
  ...
  loadLocalStorage() {
    if (!this.storage.getItem("options")) this.storage.setItem("options", JSON.stringify(this.getValuesFromForm()));
    else this.setValuesFromLocalStorage();
  }

  toggleRevealer(e) {
    this.toggle = !this.toggle;
    if (this.toggle) e.target.classList = "mobButton mine";
    else e.target.classList = "mobButton flag";
  }
  ...
  getValuesFromForm() {
    ...
    document.getElementsByName("difficulty").forEach((radio) => {
      if (radio.checked) this.difficulty = radio.value;
    });
  ...
  handleDifficultySetting() {
    ...
    if (this.difficulty === "hard") this.calculateNumberOfMines(5);
    if (this.difficulty === "easy") this.calculateNumberOfMines(10);
    if (this.difficulty === "medium") this.calculateNumberOfMines(7);
    ...
    for (let i = mine.row - 1; i < mine.row + 2; i++)
      if (i >= 0 && i < this.gameState.rows)
        for (let j = mine.column - 1; j < mine.column + 2; j++)
          if (j >= 0 && j < this.gameState.columns) if (!(i == mine.row && j == mine.column)) radius.push(new Coordinate(i, j));
    return radius;
  }
}
...
class LCD {
  ...
  getDigits(number) {
    ...
    /* 
		...
		*/
    if (returnArray[0] == "-") returnArray[0] = 11;
    while (returnArray.length < 3) returnArray.unshift(10);
    return returnArray;
  }

  incrementSeconds() {
    this.seconds++;
    if (this.seconds % 100 === 0) this.printNumber(this.seconds / 100);
  }
}
...
class Controller {
  ...
  addListeners() {
    ...
    document.querySelectorAll(".restart").forEach((x) =>
      ...
      row.forEach((column) => {
        column.addEventListener("click", (element) => {
          if (this.gameState.toggle) {
            if (this.gameState.state === "initial") this.loadGame(element.target);
            this.handleClick(element.target);
          } else {
            if (this.gameState.uncovered !== 0) this.handleRightClick(element.target);
          }
        });
        column.addEventListener("contextmenu", (flagElement) => {
          flagElement.preventDefault();
          if (this.gameState.state === "default") this.handleRightClick(flagElement.target);
        });
      })
    );
  }
  ...
  handleClick(element) {
    ...
    if (this.gameState.state == "default") {
      if (element.innerHTML != 0 && element.innerHTML < 9) this.flagReveal(element);
  ...
  easyStart(position) {
    if (this.gameState.easyStart) while (this.engine.board[position.row][position.column] != 0) this.engine.calculateBoard();
  }
  ...
  flagReveal(element) {
    let positions = this.engine.getNeighbours(this.getButtonCoordinates(element));
    let flags = 0;
    positions.forEach((x) => {
      if (this.buttons[x.row][x.column].classList.contains("flag")) flags++;
    });
    if (element.innerHTML == flags) positions.forEach((x) => this.reveal(this.buttons[x.row][x.column]));
  }

  handleRightClick(element) {
    this.setFlag(element);
    if (element.innerHTML != 0 && element.innerHTML < 9 && !this.gameState.toggle) this.flagReveal(element);
    this.gameWon();
    this.changeSun();
  }
  ...
  setEmpty(element, value) {
    ...
    if (value != 0 && !element.classList.contains("falseflag")) element.innerHTML = value;
  ...
  reveal(element) {
    ...
    const tileValue = this.engine.board[originCoordinates.row][originCoordinates.column];
    if (!this.containsClass(element, "empty") && !this.containsClass(element, "flag")) {
      if (tileValue < 9) {
        this.setEmpty(element, tileValue);
        this.engine.getNeighbours(originCoordinates).forEach((newCoordinates) => {
          if (tileValue == 0) this.reveal(this.buttons[newCoordinates.row][newCoordinates.column]);
        });
  ...
  gameOver(element) {
    this.stopTimer();
    this.playSound("files/minesweeperAssets/sound/gameOver.jpg");
    this.gameState.state = "lost";
    element.className += " deadly";
    this.engine.minePositions.forEach((x) => {
      if (!this.containsClass(this.buttons[x.row][x.column], "flag")) this.setBomb(this.buttons[x.row][x.column]);
      this.flagPositions.removePosition(x);
    });
    this.flagPositions.forEach((x) => this.setFalseFlag(this.buttons[x.row][x.column]));
  }

  // Stops the game and makes the sun smug.
  gameWon() {
    const toUncover = this.gameState.rows * this.gameState.columns - this.gameState.mines;
    if (this.gameState.uncovered == toUncover) {
      ...
      this.buttons.forEach((row) =>
        row.forEach((x) => {
          if (x.classList.length == 1) this.setFlag(x);
        })
      );
      this.playSound("files/minesweeperAssets/sound/gameWon.jpg");
      return true;
    }
    return false;
  }
  ...
  updateRemaining() {
    this.lcd.printNumber(this.gameState.mines - this.flagPositions.length, "remaining");
  }
