function buildRow(faktura) {
	...
	zaznam +=
		"'PZ','prodej zbo" +
		String.fromCharCode(167, 161) +
		" - e-shop'," +
		`1,` +
		String.fromCharCode(39, 75, 159, 39) +
		`,1,`;
	zaznam += `'${vystaveno}',`;
	zaznam += `'U',`;
	zaznam += `'${vystaveno}',`;
...
function appendPrices(faktura) {
	...
	result += `${prices.basePrices.get(15)},${prices.baseTaxes.get(
		15
	)},${prices.basePrices.get(10)},`;
	result += `${prices.baseTaxes.get(10)},${prices
		.total()
		.toFixed(2)
		.replace(/\.0+$/, "")}`;
	return result;
...
class Price {
	...
	static getFullPrice(prices) {
		return {
			...
			total() {
				return (
					[...this.basePrices.values()].reduce((x, y) => x + parseFloat(y), 0) +
					[...this.baseTaxes.values()].reduce((x, y) => x + parseFloat(y), 0)
				);
			}
		};
	}
...
function sumTaxes(prices, rate) {
	return roundNumber(
		prices.filter(x => x.taxRate == rate).reduce((x, y) => x + y.taxTotal, 0)
	);
}

function sumPrices(prices, rate) {
	return roundNumber(
		prices.filter(x => x.taxRate == rate).reduce((x, y) => x + y.taxedPrice, 0)
	);
}
...
function getNumber(node, key) {
	return parseFloat(node.querySelector(key).textContent);
}
